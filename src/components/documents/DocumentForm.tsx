...
export default function DocumentForm({
  ...
  useEffect(() => {
    if (documentToEdit) {
      ...
      setCreator(documentToEdit.creator || "");
      setFileType(documentToEdit.fileType || "");
      setFileSize(documentToEdit.fileSize || "");
      setCategory(documentToEdit.category || "");
      setTags(documentToEdit.tags || []);
      setOrder(documentToEdit.order || 0);

      // Si hay URL de documento existente, mostrarla como vista previa
      if (documentToEdit.documentUrl) {
        setFilePreview(documentToEdit.documentUrl);
      }

      // Si hay URL de miniatura existente, mostrarla como vista previa
      if (documentToEdit.thumbnailUrl) {
        setThumbnailPreview(documentToEdit.thumbnailUrl);
      }
    } else {
      setIsEditMode(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!creator.trim()) {
      setError("El creador del documento es obligatorio");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (file) {
        documentUrl = await uploadFileToStorage(file, "documents", title);
        setUploadProgress(50);
      } else if (isEditMode && documentToEdit?.documentUrl) {
      ...
      if (thumbnailFile) {
        ...
        // Mantener URL existente en modo edición
        thumbnailUrl = documentToEdit.thumbnailUrl;
      }
      ...
      const cleanedData = prepareForFirestore(documentData);
      ...
      if (isEditMode && documentToEdit?.id) {
        ...
        if (file && documentToEdit.documentUrl) {
          await deleteFileFromStorage(documentToEdit.documentUrl);
        }

        // Si se subió una nueva miniatura, eliminar la anterior
        if (thumbnailFile && documentToEdit.thumbnailUrl) {
          await deleteFileFromStorage(documentToEdit.thumbnailUrl);
        }

        console.log("✅ Documento actualizado con ID:", documentToEdit.id);
      ...
      if (errorMessage.includes("permission-denied")) {
        ...
        setError(`Error al guardar el documento: ${errorMessage}`);
      }
    } finally {
      setLoading(false);
    }
          ...
          <div className="row">
            ...
            <div className="col-12 mb-3">
              <label htmlFor="documentFile" className="form-label">
                Archivo del documento {!isEditMode && "*"}
              </label>
              <input
                ref={fileInputRef}
                type="file"
            ...
            <div className="col-md-6 mb-3">
              ...
              />
              <div className="form-text">
                Persona o departamento que creó el documento
              </div>
            </div>
            ...
            <div className="col-12 mb-3">
              ...
              <textarea
                ...
                placeholder="Describe el contenido del documento, su propósito, versión, etc."
                disabled={loading}
              />
            </div>
              ...
              <button
                ...
                onClick={() => {
                  if (isEditMode && documentToEdit) {
                    ...
                    setDescription(documentToEdit.description || "");
                    setFile(null);
                    setFilePreview(documentToEdit.documentUrl || null);
                    setThumbnailFile(null);
                    setThumbnailPreview(documentToEdit.thumbnailUrl || null);
                    setCreator(documentToEdit.creator || "");
                    setFileType(documentToEdit.fileType || "");
                    setFileSize(documentToEdit.fileSize || "");
                    setCategory(documentToEdit.category || "");
